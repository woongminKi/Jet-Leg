...
def _call_search(query: str, doc_id: str, limit: int = 50) -> dict:
    """search() 직접 호출 — doc_id 스코프 + 모든 매칭 chunks 노출.

    doc_id 명시 시 _RPC_TOP_K_DOC_FILTER (200) + chunk cap _MAX_MATCHED_CHUNKS_DOC_SCOPE
    로 동작 — chunk_idx 순서대로 매칭 청크 다수 응답.
    """
