...
> W2 스프린트 KPI 측정 시드 + W25 D7 mini-Ragas (Phase 1) 도입.
> **상태 (W25 D7 마감)**: golden v0.3 ship (top-1 100%, 20/20) + mini-Ragas (10 QA, Context Recall 100% / Precision 0.730) ship.
> Phase 2 (135 QA / Faithfulness / Answer Relevancy) 는 사용자 자료 누적 + LLM answer 어댑터 결정 후.

## W25 D7 — mini-Ragas (Phase 1) 진입 결과

| 항목 | 값 | 사용자 의도 대비 |
|---|---|---|
| 데이터셋 | `golden_v0.4_sonata.csv` (10 QA, SONATA 1건) | 사용자 자료 누적 차단 회피 |
| Context Recall@10 (평균) | **1.000 (100%)** | 70~90% 기대 → **상회** |
| Context Precision@10 (평균) | **0.730** | precision 격차 (0.05~1.00) → 후속 ranking 개선 신호 |
| latency p95 | 624ms | 정상 범위 |

실행:
```bash
make eval                              # Makefile entry-point (DoD ③)
# 또는 직접:
cd api && uv run python ../evals/run_ragas.py --top_k 10
```

자세한 결과 — `work-log/2026-MM-DD ragas-mini-result.md` (실행 시 자동 생성).
...
**1단계: 의존성 승인 — W25 D7 완료**

```bash
cd api && uv add ragas datasets
# → ragas 0.4.3 / datasets 4.8.5 (+ pandas / numpy / transitive)
```

W11~W24 누적 외부 의존성 0 정책 첫 변경 — W25 D7 sprint 에서 사용자 명시 승인.
